...
class MAPPOShareDiscreteAgentTrainer(AgentTrainer):
    ...
    def __init__(self, config, algo_config, agent_index, agents):
        ...
        assert not (self.use_popart and self.use_valuenorm), "use_popart and use_valuenorm must set True simultaneously"
    ...
    def update(self, agents, train_step, agent_index=None):
        # compute return
        all_trajectory_data, all_rnn_data, all_join_obs = Batch(), Batch(), Batch()
        for agent in agents:
            trajectory_data, rnn_trajectory_data, join_obs = self.compute_return(agents, agent.agent_index)
            # 需要在batch_size维度(dim=0)拼接
            all_trajectory_data.cat_(trajectory_data)
            all_rnn_data.cat_(rnn_trajectory_data)
            all_join_obs.cat_(join_obs)

        # train network
        info = self.train_network(all_trajectory_data, all_rnn_data, all_join_obs)
    ...
    def compute_return(self, agents, agent_index):
        ...
        agents[agent_index].replay_buffer.compute_return(next_value, self.value_normalizer)
    ...
    def rnn_forward_generator(self, buffer, rnn_buffer, advantages, join_obs, data_chunk_length):
        ...
        for indices in sampler:
            ...
            yield share_obs_batch, obs_batch, actor_rnn_hidden_batch, critic_rnn_hidden_batch,\
    ...
    def feed_forward_generator(self, buffer, advantages, join_obs, mini_batch_size=None, rnn_buffer=None):
        ...
        actor_rnn_hidden_state, critic_rnn_hidden_state, available_actions = [], [], []
        ...
        for indices in sampler:
            ...
            yield share_obs_batch, obs_batch, actor_rnn_hidden_state_batch, critic_rnn_hidden_state_batch, \
                actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch,\
                old_action_log_probs_batch, adv_targ, available_actions_batch
