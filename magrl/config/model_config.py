...
envs_paths = {"uavs": os.path.join(env_prepath, r"custom_env\uav_5v5\uav_5v5.py"),
              "atsc": os.path.join(env_prepath, r"custom_env\atsc\atsc.py"),
              }
...
def set_global_random_seed(seed):
    ...
    try:
        ...
        logger.warning(f"no have tensorflow, no need set tf.random.seed. {e}")

    try:
        import torch
        torch.manual_seed(seed)
    except Exception as e:
        logger.warning(f"no have torch, no need set torch.manual_seed. {e}")
