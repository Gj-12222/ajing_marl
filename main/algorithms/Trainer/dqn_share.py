...
class DQNShareAgentTrainer(AgentTrainer):
    ...
    def update(self, trainers, train_step, agent_index=None):
        ...
        trainer_data = self.replay_buffer.sample_index(indexs)
        trainer_data.to_torch(device=self.device)

        mask = self.algo_cfg['gamma'] * (1.0 - trainer_data.done)
        q_values = self.QNetCritic(trainer_data.obs).gather(1, trainer_data.action)  # Q值
        max_next_q_values = self.targetQNetCritic(trainer_data.next_obs).max(1)[0].view(-1, 1)  # 下个状态的最大Q值
        q_targets = trainer_data.reward + mask * max_next_q_values  # TD目标
        ...
        info = {'q_value': torch.mean(q_values).item(),
                ...
                'epsilon': self.epsilon,
                'batch_reward': torch.mean(trainer_data.reward).item()}
        return info
