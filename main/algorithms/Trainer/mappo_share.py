...
from algorithms.rl_utils.ReplayBuffer import ReplayBufferTrajectory, SampleIndexs
from algorithms.rl_utils.batch import Batch
from algorithms.rl_utils.valuenorm import ValueNorm


class MAPPOShareDiscreteAgentTrainer(AgentTrainer):
    name = 'mappo_share'
    ...
    def __init__(self, config, algo_config, agent_index, agents):
        ...
        self.PolicyActor = DiscreteRNNActor(self.obs_dim, self.hidden_dim, sum(self.action_dim), device=self.device,
                                            cfg=algo_config)
        self.targetPolicyActor = DiscreteRNNActor(self.obs_dim, self.hidden_dim, sum(self.action_dim),
                                                  device=self.device, cfg=algo_config)
        ...
        self.targetVNetCritic = RNNMLPCritic(self.v_input_dim, self.hidden_dim, 1, device=self.device, cfg=algo_config)
    ...
    def action(self, obs_n):
        ...
        mask = self.replay_buffer._storage.mask[self.replay_buffer._next_idx]
        if self.use_recurrent_policy:
            _actor_rnn_hidden_state = self.replay_buffer._rnn_storage.actor_rnn_hidden_state[self.replay_buffer._next_idx]
            _critic_rnn_hidden_state = self.replay_buffer._rnn_storage.critic_rnn_hidden_state[self.replay_buffer._next_idx]
        else:
        ...
        self.action_log_prob = action_dist.log_prob(action).sum(dim=-1)
    ...
    def save_replay_buffer(self, obs, action, reward, next_obs, done, terminal):
        ...
        mask[np.array([done or terminal], dtype=np.float32).reshape(1, -1) == True] = \
            np.zeros(((np.array([done or terminal], dtype=np.float32).reshape(1, -1) == True).sum(),), dtype=np.float32)
    ...
    def update(self, agents, train_step, agent_index=None):
        # compute return
        trajcetory_data, rnn_trajcetory_data, join_obs = self.compute_return(agents)

        # train network
        info = self.train_network(trajcetory_data, rnn_trajcetory_data, join_obs)
    ...
    def compute_return(self, agents):
        trajcetory_data, rnn_trajcetory_data, join_obs = None, None, []
        for i in range(self.n):
            ...
            if self.agent_index == i:
                ...
                trajcetory_data = _buffer_data
                rnn_trajcetory_data = _rnn_buffer_data

        join_obs = torch.cat(join_obs, dim=-1)
        if self.use_recurrent_policy:
            critic_rnn_hidden_state = rnn_trajcetory_data.critic_rnn_hidden_state[-1].reshape(1, -1)
        else:
            critic_rnn_hidden_state = None
        next_value, _ = self.VNetCritic(join_obs[-1].unsqueeze(dim=0),
                                        critic_rnn_hidden_state,
                                        trajcetory_data.mask[-1].reshape(1, -1))
        next_value = self._t2n(next_value).reshape(1, -1)
        self.replay_buffer.compute_return(next_value, self.value_normalizer)

        return trajcetory_data, rnn_trajcetory_data, join_obs
    ...
    def rnn_forward_generator(self, buffer, rnn_buffer, advantages, join_obs, data_chunk_length):
        ...
        if len(join_obs.shape) > 3:
            join_obs = join_obs[:-1].transpose(1, 0, 2, 3, 4).reshape(-1, *join_obs.shape[2:])
            obs = buffer.obs[:-1].transpose(1, 0, 2, 3, 4).reshape(-1, *buffer.obs.shape[2:])
        else:
            join_obs = join_obs[:-1]
            obs = buffer.obs[:-1]
        ...
        for indices in sampler:
            share_obs_batch = []
            obs_batch = []
            actor_rnn_hidden_state_batch = []
            critic_rnn_hidden_state_batch = []
            actions_batch = []
            available_actions_batch = []
            value_preds_batch = []
            return_batch = []
            ...
            for index in indices:
                ...
                actor_rnn_hidden_state_batch.append(actor_rnn_hidden_state[ind])
                critic_rnn_hidden_state_batch.append(critic_rnn_hidden_state[ind])
            ...
            actor_rnn_hidden_state_batch = torch.stack(actor_rnn_hidden_state_batch).reshape(N,
                                                                                          *rnn_buffer.actor_rnn_hidden_state.shape[
                                                                                           1:])
            critic_rnn_hidden_state_batch = torch.stack(critic_rnn_hidden_state_batch).reshape(N,
                                                                                            *rnn_buffer.actor_rnn_hidden_state.shape[
                                                                                             1:])
    ...
    def feed_forward_generator(self, buffer, advantages, join_obs, mini_batch_size=None, rnn_buffer=None):
        ...
        if self.use_recurrent_policy:
            actor_rnn_hidden_state = rnn_buffer.actor_rnn_hidden_state[:-1].reshape(-1,
                                                                                *buffer.actor_rnn_hidden_state.shape[
                                                                                 1:])
            critic_rnn_hidden_state = rnn_buffer.critic_rnn_hidden_state[:-1].reshape(-1,
                                                                                  *buffer.critic_rnn_hidden_state.shape[
                                                                                   1:])
    ...
    def evaluate_actions(self, cent_obs, obs, actor_rnn_hidden_state, critic_rnn_hidden_state, action, masks,
    ...
    @property
    def model(self):
        return [self.VNetCritic, self.VNetCritic_optimizer]

    @model.setter
    def model(self, NetWorkList):
        print(self.agent_index, 'is setter Network!')
        self.VNetCritic, self.VNetCritic_optimizer = NetWorkList
    ...
    def save_model(self, path):
        if not os.path.exists(path):
            os.makedirs(path)
        save_name = ['PolicyActor', 'PolicyActor_optimizer', 'VNetCritic', 'targetVNetCritic', 'VNetCritic_optimizer']
        for _save_name in save_name:
            torch.save(getattr(self, _save_name), f"{path}/{_save_name}.pt")

    def load_model(self, path):
        load_name = ['PolicyActor', 'PolicyActor_optimizer', 'VNetCritic', 'targetVNetCritic', 'VNetCritic_optimizer']
