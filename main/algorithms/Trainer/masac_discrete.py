...
class MASACDiscreteAgentTrainer(AgentTrainer):
    name = 'masac_discrete'
    def __init__(self, config, algo_config, agent_index, agents):
        ...
        self.target_entropy = - np.log(1 / sum(self.action_dim))
    ...
    def update(self, agents, train_step, agent_index=None):
        ...
        # alpha
        alpha_loss = (-act_prob.detach() * (self.log_alpha *
                                            (log_act_prob + self.target_entropy).detach())).sum(dim=-1).mean()
