...
class MATD3DiscreteAgentTrainer(AgentTrainer):
    name = 'matd3'
    def __init__(self, config, algo_config, agent_index, agents):
        ...
        self.delay_update_freq = self.algo_cfg['delay_update_freq']
        ...
        self.targetPolicyActor = SoftDiscreteActor(self.obs_dim, self.hidden_dim, sum(self.action_dim),  device=self.device)
    ...
    def update(self, agents, train_step, agent_index=None):
        #  > batch_size * 10
        if len(self.replay_buffer) < self.algo_cfg['batch_size'] * 10:
            info = {'q_value': 0.0,
                    'q_loss': 0.0,
                    'p_loss':0.0,
                    'batch_reward': 0.0}
        ...
        # add noise
        next_action_n =torch.cat([agent.targetPolicyActor(next_obs).sample().detach() for agent, next_obs in zip(agents, next_obs_n)],
                                 dim=-1)
        ...
        # delay update actor
        if train_step % (2 * self.delay_update_freq):
            action_dist = self.PolicyActor(agent_data.obs)
            action_n[self.agent_index] = action_dist.sample()
            action_n = torch.cat(action_n, dim=-1)
            # use cri_target is more stable than cri
            q_value = self.targetQ1NetCritic(torch.cat([obs_n, action_n], dim=-1)) + self.targetQ2NetCritic(torch.cat([obs_n, action_n], dim=-1))
            policy_loss = - torch.mean(q_value / 2)
