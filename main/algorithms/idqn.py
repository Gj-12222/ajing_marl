...
def q_train(make_obs_n,
    ...
    with tf.variable_scope(scope,reuse=False):
        ...
        q_predit = tf.gather_nd(q, action_index_ph)
        ...
        q_target_max_values = U.function(inputs=[obs_ph_n[agent_index]],outputs=target_q_max)
...
class IDQNAgentTrainer(AgentTrainer):
    def __init__(self, name, obs_shape_n, act_space_n, agent_index, args, local_q_func=False):
        super(IDQNAgentTrainer, self).__init__(name, obs_shape_n, act_space_n, agent_index, args, local_q_func)
        # ①定义变量
        self.name=name
    ...
    def update(self, agents, t):
        ...
        for i in range(num_sample):
            q_target_next_max = self.q_target_values(obs_next_n[self.agent_index])
            # 计算Q值 Q = R + gamma * (1 - done——终止条件) * Q
            target_u += rew + self.args.gamma * (1 - done) * q_target_next_max
        target_u /= num_sample
        q_probs = self.q_probs_sample(obs_n[self.agent_index])
