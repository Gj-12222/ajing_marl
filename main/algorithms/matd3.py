...
def p_train(make_obs_ph_n, act_space_n, p_index, p_func, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        p_func_va = U.absolute_scope_name("p_func")
        p_scope = U.absolute_scope_name(scope)
        p_func_vars = U.scope_vars(p_func_va)
...
def q_train(make_obs_ph_n, act_space_n, q_index, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, scope="trainer", reuse=None, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Create callable functions
        train1 = U.function(inputs=obs_ph_n + act_ph_n + [target_ph], outputs=loss, updates=[optimize1_expr])
        train2 = U.function(inputs=obs_ph_n + act_ph_n + [target_ph], outputs=loss, updates=[optimize2_expr])
        ...
        return [train1,tarin2], [update_target_q1,update_target_q2], {'min_q_values': min_q_values, 'min_target_q_values': target_q_values}
...
class MATD3AgentTrainer(AgentTrainer):
    ...
    def experience(self, obs, act, rew, new_obs, done, terminal):
        ...
        self.replay_buffer.add(obs, act, rew, new_obs, 1.0 if done==True or terminal==True else 0.0)
    ...
    def update(self, agents, t):
        ...
        for i in range(num_sample):
            target_act_next_n = [agents[i].p_debug['target_act'](obs_next_n[i]) for i in range(self.n)]  # 在计算图里给目标策略网络带了噪声
