...
class Scenario(BaseScenario):
    ...
    def compute_lock_num(self, agent, world):
        ...
        for i, opp in enumerate(opponent):
            capture,flag = self.is_collision(opp,agent)
            if flag == 1:
                agent.lock_num[i] += 1
            else:
                agent.lock_num[i] = 0
    ...
    def is_collision(self, agent1, agent2):
        ...
        agent1_cross = (delta_pos[0]*agent1.state.p_vel[0]+delta_pos[1]*agent1.state.p_vel[1])/(distance)
        if agent1_cross < -1:
           agent1_cross  = -1
        if agent1_cross > 1:
           agent1_cross = 1
        ...
        agent2_cross = (-delta_pos[0]*agent2.state.p_vel[0]-delta_pos[1]*agent2.state.p_vel[1])/(distance)
        if agent2_cross < -1:
           agent2_cross  = -1
        if agent2_cross > 1:
           agent2_cross = 1
        ...
        if distance < fire_range and agent2_angle*180/math.pi>defense_angle and agent1_angle*180/math.pi<attack_angle/2:
            return True,1
        elif distance < fire_range and agent2_angle*180/math.pi<attack_angle/2 and agent1_angle*180/math.pi>defense_angle:
            return True,2
        else:
            return False,0
