...
def regist_algorithm(algorithm_name):
    algorithm_path = algorithm_paths.get(algorithm_name, None)
    config_path =  config_paths.get(algorithm_name, None)
    if algorithm_path and config_path:
        moduleTrainer = importlib.import_module(algorithm_path[0])
        trainer = getattr(moduleTrainer, algorithm_path[1])

        moduleCfg = importlib.import_module(config_path[0])
        cfg = getattr(moduleCfg, config_path[1])
